...
def make_inference_for_dataset(path_of_dataset):
    ...
    input_ids_left, input_ids_right, attention_masks_left, attention_masks_right, subword_spans_left, subword_spans_right, list_token_index_of_sentence_left, list_token_index_of_sentence_right,tokens_left, tokens_right = get_input_ids_and_so_on(tokenizer, path_of_dataset, 64)
    ...
    model.load_state_dict(torch.load('computational-annotator-pilot/xmlr+mlp-naive.pth'))
    with torch.no_grad():
...
def make_inference_for_single_instance(path_of_dataset):
    '''use GPU if possible'''
    # If there's a GPU available...
    if torch.cuda.is_available():    
    ...
    tokenizer = get_XLMRTokenizer()
    get_max_sentence_length_of_a_dataset_by_tokenizer(tokenizer, path_of_dataset)
    input_ids_left, input_ids_right, attention_masks_left, attention_masks_right, subword_index_list_left, subword_index_list_right = get_input_ids_and_so_on(tokenizer, path_of_dataset)
    index_left_padded, index_right_padded = get_padded_index(tokenizer, path_of_dataset, subword_index_list_left, subword_index_list_right)

    model = XLMRobertaModel.from_pretrained('xlm-roberta-base', output_hidden_states=True)
    save_embeddings(device, input_ids_left, attention_masks_left, index_left_padded, 'tmp/token_embeddings_left.npy', model)

    model = XLMRobertaModel.from_pretrained('xlm-roberta-base', output_hidden_states=True)
    save_embeddings(device, input_ids_right, attention_masks_right, index_right_padded, 'tmp/token_embeddings_right.npy', model)
...
# make_inference_for_dataset('tmp/instances_wic.csv')