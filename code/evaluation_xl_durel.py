...
import pandas as pd
from xl_durel import specify_xl_durel_annotator
import json
import logging
...
def evaluate_xl_durel(custom_dir,custom_filename,usage_dir,thresholds,debug,dataset): # to do: it would be better to specify the metric as input
        annotator = specify_xl_durel_annotator(thresholds)
        ...
        if annotator in ["XL-DURel-Binary"]:
...
def save_detailed_xl_durel_results(df_sorted_gold,df_sorted_pred,usage_dir,custom_dir,dataset,annotator,accuracy,correlation,p_value,debug):
    if debug:
