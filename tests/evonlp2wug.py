...
'''
lemma2instances = defaultdict(lambda: [])
for row in table:
    lemma = row[0].split('-')[1]
    id1 = row[0]
    id2 = row[0]
    ...
    judgment = '0.0,1.0'
    annotator = np.nan
    data = {'identifier1':id1+'-tweet1','identifier2':id2+'-tweet2','label-set':float(judgment),'lemma':lemma}
    lemma2instances[lemma].append(data)
'''
...
#with open(all_output_folder +'instances.csv', 'w') as f:
#    w = csv.DictWriter(f, [lemma2instances[lemma] for lemma in lemma2instances][0][0].keys(), delimiter='\t', quoting = csv.QUOTE_NONE, quotechar='')
#    w.writeheader()
#    for lemma in lemma2instances:
