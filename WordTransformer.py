...
class WordTransformer(nn.Sequential):
    ...
    def encode(self, sentences: Union[str, List[str]],
        ...
        :param output_value:  Default sentence_embedding, to get sentence embeddings. Can be set to token_embeddings to get wordpiece token embeddings. Set to None, to get all output values, or to target_embedings to get embeddings of the target_word at positions
        ...
        for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            features,target_indices = self.tokenize(sentences_batch)
            ...
            with torch.no_grad():
                ...
                elif output_value == 'target_embeddings':
                    embeddings = []
        ...
        if output_value == 'target_embeddings':
            target_embedings = [all_embeddings[0][t_idx] for t_idx in target_indices]
            target_embedings = np.asarray([emb.numpy() for emb in target_embedings])
            all_embeddings = target_embedings
    ...
    def tokenize_batch(self,batch,sentid=None):
        ...
        for example in batch:
            ...
            target_indices = [i+2 for i in range(len(tokens)) if tokens[i] == '<t>']
            #print('tokens',tokens, target_indices)
        ...
        return tokenized,target_indices
